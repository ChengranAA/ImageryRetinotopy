...
# Ring-grid parameters (as used in 00_generate_ring_grid.py)
NUM_RINGS = 6
OUTER_RADIUS = 1.0
INNER_RADIUS = OUTER_RADIUS / (NUM_RINGS * 2.0)


def generate_annular_rings(
    ...
    """
    Generate ring masks as annuli spanning adjacent radial segments.
    """
    out_dir = os.path.join(OUTPUT_ROOT, OUTPUT_SUBDIR)
    os.makedirs(out_dir, exist_ok=True)

    # Radii used by 00_generate_ring_grid.py to draw ring outlines.
    ...
    def save_annular_mask(inner_radius, outer_radius, mask_index):
        ...
        filename = f"ring_{mask_index:02d}.png"
        ...
        ring_masks[f"ring_{mask_index:02d}"] = alpha_mask
    ...
    pkl_path = os.path.join(out_dir, "all_rings.pkl")
